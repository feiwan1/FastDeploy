...
export FLAGS_selected_intel_hpus=7
rm -rf log 2>/dev/null
HPU_PERF_BREAKDOWN_SYNC_MODE=1 HPU_WARMUP_BUCKET=1 HPU_WARMUP_MODEL_LEN=4096 FD_ATTENTION_BACKEND=BLOCK_ATTN python -m fastdeploy.entrypoints.openai.api_server --model baidu/ERNIE-4.5-21B-A3B-Paddle --port ${SERVER_PORT} --engine-worker-queue-port ${ENGINE_WORKER_QUEUE_PORT} --metrics-port ${METRICS_PORT} --tensor-parallel-size 1 --max-model-len 32768 --max-num-seqs 128 --block-size 128
